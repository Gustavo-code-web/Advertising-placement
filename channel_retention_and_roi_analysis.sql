...
渠道,avg(曝光) 曝光_前
from `渠道行为表`
where 日期 between '2019-10-29' and '2019-11-04' 
GROUP BY 渠道) a,
(
select
渠道,avg(曝光) 曝光_中
from `渠道行为表`
where 日期 between '2019-11-05' and '2019-11-11' 
GROUP BY 渠道) b,
(
select
渠道,avg(曝光) 曝光_后
from `渠道行为表`
where 日期 between '2019-11-12' and '2019-11-18' 
GROUP BY 渠道) c
...
t2 as (
select
id,DATEDIFF('2019-12-13',日期) 注册天数
from `用户信息表`
),
...
id,sum(订单数量) 总单
from `用户行为表`
where 日期 between '2019-01-01' and '2019-12-13'
group by id
),
t3 as (
select 
...
-- 2. 为每个月分配一个序号（2019-01为1，2019-02为2...2019-11为11）
month_sequence AS (
    SELECT *,
...
-- 5. 预测2020年11月的CPA（2020年11月对应的month_num为：11月+12个月=23）
SELECT 
    渠道,
    ROUND(intercept + slope * 23, 2) AS predicted_cpa_nov_2020,
...
-- 4. 预测2020年11月的CPA（2020年11月对应的month_num为：11月+12个月=23）
SELECT 
    渠道,
